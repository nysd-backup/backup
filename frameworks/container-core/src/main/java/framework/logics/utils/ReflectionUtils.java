...
public abstract class ReflectionUtils {
	...
	/**
	 * @param leafClass 枝クラス
	 * @param findClass 検索終了アノテーション(含む)
	 * @return フィールド
	 */
	public static Field[] getDeclaredFields(Class<?> leafClass , Class<? extends Annotation> findStop) {

		List<Field> ls = new ArrayList<Field>();

		for (Class<?> cls = leafClass; cls != Object.class; cls = cls.getSuperclass()) {		
			...
			if( cls.getAnnotation(findStop) != null){
				break;
			}	
		}
	...
	public static Field[] getDeclaredFields(Class<?> leafClass) {
		return getDeclaredFields(leafClass,null);
	}
	 ...
	 * @throws IllegalStateException if the bean cannot be instantiated
	 */
	public static <T> T instantiate(Class<T> clazz) {
	
		if (clazz.isInterface()) {
			throw new IllegalStateException("Specified class is an interface :" + clazz );
		}
		try {
	 ...
	 * @return the corresponding Field object, or <code>null</code> if not found
	 */
	public static Field findRequiredDeclaredField(Class<?> clazz, String name) {
		Field f = findDeclaredField(clazz,name);
		if( f == null){
