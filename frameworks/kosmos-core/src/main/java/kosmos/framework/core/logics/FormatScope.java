 ...
 * @version	created.
 */
public interface FormatScope {

	/** for StringFormatScope */
	interface StringScope extends FormatScope {
		/** カラム名の区切り文字 */
	...
	/** for ObjectFormatScope */
	interface ObjectScope extends FormatScope {
		/** 行セパレータ */
	...
	/** for DateFormatScope */
	interface DateScope extends FormatScope {
		/** 固定日付 */
	...
	/** for BigDecimalScope */
	interface BigDecimalScope extends FormatScope {
		interface Formats {
			/** 数値(カンマ無し) */
	...
	interface SpecialScope extends FormatScope {
		enum Formats {
			/** 郵便番号 */
			PostalCode,
			/** 電話番号 */
