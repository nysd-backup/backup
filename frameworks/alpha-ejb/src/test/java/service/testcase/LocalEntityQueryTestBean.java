...
public class LocalEntityQueryTestBean extends BaseCase {
	...
	public void isNullisNotNull(){
		...
		UpdateQuery<TargetEntity> modifier = createUpdater(TargetEntity.class);
	...
	public void duplicateError(){
		...
		//2件目
		TargetEntity e2 = new TargetEntity();
		e2.setAttr("attr");
		e2.setTest("1");
		em.persist(e2);
	...
	public void allCondition(){
		...
		ListReadQuery<TargetEntity> query = createListReader(TargetEntity.class);	
		query.setHint(QueryHints.HINT,"/*+ HINT */");
		
		List<TargetEntity> result = getOneRecord(query);
	
		assertEquals(1,result.size());
		
		TargetEntity first = result.get(0);
		em.detach(first);
		first.setAttr("100");
		
		ListReadQuery<TargetEntity> forres = createListReader(TargetEntity.class);
		List<TargetEntity> updated = getOneRecord(forres);	
	...
	public void disableDetach(){
		...
		//更新結果
		SingleReadQuery<TargetEntity> forres =createSingleReader(TargetEntity.class);
		TargetEntity entity = forres.eq(ITestEntity.TEST, "2").call();
	...
	public void updateAny(){
		...
		//更新
		UpdateQuery<TargetEntity> update = createUpdater(TargetEntity.class);
		...
		TargetEntity entity = query.eq(ITestEntity.TEST, "2").call();
...
//	
	...
	public void getResultSetFirst(){
		...
		ListReadQuery<TargetEntity> query = createListReader(TargetEntity.class);
		query.setFirstResult(1);
		List<TargetEntity> result = query.call();
		assertEquals(1,result.size());
...
//	}
	...
	public void find(){	
		...
		result = em.find(TargetEntity.class,"1");
...
//	}
	...
	public void isEmptyPK(){
		setUpData("TEST.xls");
		assertEquals(false,em.find(TargetEntity.class,"AGA") != null);
		context.setRollbackOnly();
	}
	...
	public void existsPK(){
		...
		assertEquals(true,em.find(TargetEntity.class,"1") != null);
		context.setRollbackOnly();
	}
	...
	public void ignoreUniqueConstraintError(){
		...
		TargetEntity entity2 = new TargetEntity();
		entity2.setTest("530");
		entity2.setAttr("aaa");
		entity2.setAttr2(30);
	...
	public void versionNoError(){	
		setUpData("TEST.xls");
		final TargetEntity result = em.find(TargetEntity.class,"1");
		result.setVersion(2);
		try{
			em.flush();
			fail();
...
//		}
		...
		TargetEntity res2 =  em.find(TargetEntity.class,"2");
		assertEquals(0,res2.getVersion());
		res2.setAttr("aa");
		em.flush();

		assertEquals("aa", em.find(TargetEntity.class,"2").getAttr());
		context.setRollbackOnly();
	}
	 ...
	 * @return
	 */
	private List<TargetEntity> getOneRecord(ListReadQuery<TargetEntity> query ){	
